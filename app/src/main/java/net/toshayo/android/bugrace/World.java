...
public class World implements IDrawable {
    ...
    public void draw(Canvas canvas) {
        if(_world == null) {
            _world = Bitmap.createBitmap(canvas.getWidth(), canvas.getHeight(), Bitmap.Config.ARGB_8888);
            Canvas worldCanvas = new Canvas(_world);
            for(int i = 0; i < canvas.getHeight(); i += _worldChunkSprite.getIntrinsicHeight()) {
